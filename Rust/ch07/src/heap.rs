...
impl Heap {
    pub fn create(initial_size: usize) -> Heap {
        let h = Heap {
    ...
    pub fn insert(&mut self, new_data: ElementType) {
    ...
    pub fn delete_min(&mut self, root: &mut HeapNode) {
...
mod tests {
    ...
    #[test]
    fn delete_min_returns_ascending() {
        let mut h = Heap::create(3);
        for v in [12, 87, 111, 34, 16, 75] {
            h.insert(v);
        }
        assert_eq!(h.used_size, 6);

        let mut out = Vec::new();
        let mut min_node = HeapNode::default();
        for _ in 0..6 {
            h.delete_min(&mut min_node);
            out.push(min_node.data);
        }
        assert_eq!(out, vec![12, 16, 34, 75, 87, 111]);
        assert_eq!(h.used_size, 0);
    }

    #[test]
    fn root_is_minimum_after_inserts() {
        let mut h = Heap::create(3);
        for v in [50, 30, 70, 10, 90] {
            h.insert(v);
        }
        assert_eq!(h.nodes[0].data, 10);
    }
}
